...
<template>
  <div class="md:text-4xl text-lg font-bold text-center text-slate-200">
    ...
    <a href="mailto:tdignoire@pm.me" title="Envoyer un mail à tdignoire@pm.me"
      class="group md:text-5xl text-4xl text-blue-400 transition ease-in-out duration-200">N'hésitez pas <span
        aria-hidden="true"
        class="inline-block translate-x-0 group-hover:translate-x-3 transition-transform ease-in-out duration-200">📨</span></a>
  </div>
  <div class="flex gap-4 justify-center items-center pt-16 pb-24">
    <div>
      <button data-popover-target="popover-click" data-popover-trigger="click" data-popover-placement="bottom"
        ...
        type="button" @click="handleClick" title="Copier mon adresse e-mail">
    ...
    <a href="https://gitlab.com/iDrack" title="Accéder à mon GitHub.">
      <button
        ...
        <span class="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
          <FontAwesomeIcon :icon="['fab', 'gitlab']" size="xl"
            class="transition-transform duration-200 hover:scale-125" />
        </span></button>
    </a>
  ...
  <div data-popover id="popover-click" role="tooltip"
    class="absolute z-10 invisible inline-block w-72 text-sm transition-opacity duration-300 border rounded-lg shadow-sm opacity-0 text-gray-400 border-gray-600 bg-gray-800">
